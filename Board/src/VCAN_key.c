 ...
 * 定义 KEY 编号对应的管脚
 */
PTXn_e KEY_PTxn[KEY_MAX] = {NaviOK, NaviUp, NaviDown, NaviLeft, NaviRight, PTA6, PTA7, PTE12, PTE11, PTE10, PTE9, PTE8, PTE7, PTE6, PTD15};

/*
*存放按键的状态
*/
uint16 KEY_ALL = 0;
uint16 KEY_ALL_HOLD = 0;
...
void key_IRQHandler(void)
{
    ...
    for(keynum = (KEY_e)0 ; keynum < KEY_MAX; keynum ++)    //每个按键轮询
    {
        if(key_get(keynum) == KEY_DOWN)                     //判断按键是否按下
        {
            ...
            {
                keytime[keynum] = KEY_HOLD_TIME + 1;        //继续重复检测 hold 状态
            }
        }
        else
        {
            ...
            //{
                ...
                key_update(keymsg);
        //    }
            KEY_ALL_HOLD &= ~(1<<keymsg.key);                   //弹起则将HOLD状态清空
            keytime[keynum] = 0;                            //时间累计清0
        }
    }
}

void key_update(KEY_MSG_t key_msg){
    if(key_msg.status == KEY_HOLD){
        KEY_ALL_HOLD |= (1<<(key_msg.key));     //hold置位
    }
    else{
        if(key_msg.status == KEY_DOWN){
            KEY_ALL &= ~(1<<key_msg.key);       //按下置位
            //KEY_ALL_HOLD &= (0xfffe<<key_msg.key);
            //lcd_showuint16(0,9,KEY_ALL_HOLD);
        }else if(key_msg.status == KEY_UP){
            KEY_ALL |= (1<<key_msg.key);        //弹起置位
            //KEY_ALL_HOLD &= (0xfffe<<key_msg.key);
            //lcd_showuint16(0,9,KEY_ALL_HOLD);
        }
    }
...
*
*/
uint8   get_bmkey_value(void){
    uint8 value = 0;
    for(int i = KEY_8;i >= KEY_1;i--){
        value = value<<1;       //最后一次不用移位
        value |= key_get(i);
        KEY_ALL |= (key_get(i))<<i;     //顺带更新KEY_ALL
    }
    return value;
}
