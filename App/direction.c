...
/**********全局变量定义********/ 
float Direction_P = 61;	//方向控制P  提高P偏差响应变快 如果发现转弯及时但是过不了就应增大P
float Direction_D = 22;	//方向控制D
float DirectionErr[2][5] = {0};	//方向偏差（g_fDirectionError[0]为一对水平电感的差比和偏差）               误差大于0则处于铜线左侧！！！
...
int16 sensor[10][SENSOR_SIZE]={0};       //获取的电感值
...
void Direction_Data_Update(void){
    Read_ADC();
    DirectionErr[0][0] = (my_sqrt((float)(sensor[2][0]),0)-my_sqrt((float)(sensor[3][0]),0))/(sensor[2][0] + sensor[3][0]);
    for(int i=4;i>=1;i--){
...
void Direction_Control(void){
    ...
    /****停车保护****/
    if(sensor[ADPR][0]<10&&sensor[ADVR][0]<10){ //计数使结果更精确
        Stop_Count++;
        if(Stop_Count>=7){
            Stop_Flag = 1; //停车
        }
    ...
    /* * * * * * * * * * 长直道判断* * * * * * * * * /
     ...
     if(Miss_Flag==1&&zhijiao_Flag == 1){
        ...
        Setted_Speed = 30;
        //if()zhijiao_Flag = 0;
        if(sensor[4]==10&&zhijiao_Flag == 0)zhizuo_Flag = 0;
        printf("直角\n");
        return;
     } */
...
void Read_ADC(){
    ...
    for(ADC_e i = ADP1;i < AD_MAX;   i++){//一字电感
        ...
        sensor[i][0] = (int16)(ad_val[i][0]/10*10);  //暂时不做降精度处理.ps:以后或许会用到(出自13届代码)
